...
结构校验（阻断性）：
  ...
  ✓ 所有边的目标节点在节点列表中存在
  ✓ 无循环引用
  ✓ 所有 process 节点有 line_range 或 section_heading 或 description 非空
  ✓ 每个节点至少有一条出边，或显式标记为 END
  ✓ node_type 值必须是 5 种之一：human_observation / api_call / remote_action / physical_action / decision

node_type 合理性校验（warning，不阻断）：
  ⚠ 节点 description 包含物理关键词（拔/插/断电/更换/拆/装/按下）
    但 node_type 不是 physical_action → 提示 Agent 1 确认
  ⚠ 节点 description 包含查询/检查/读取关键词
    但 node_type 是 remote_action → 可能应为 api_call
```
...
Agent 2 定位原文内容时，按优先级链执行（详见 §原文定位策略）：

```
line_range（Agent 1 记录的行号范围）→ 最精准，第一优先级
  ↓ 不存在或不匹配
section_heading（Grep 标题定位）→ 标准路径
  ↓ 为 null
description 关键词 Grep → prose 文档 fallback
  ↓ 全部失败
frontmatter 标注 content_source: match_failed → 进入 ImportReport.warnings
```

`line_range` 的引入大幅降低了 prose 文档（无标题）的定位失败率。Agent 1 在通读阶段天然知道每段内容的位置，只需在 DAG 节点中记录行号即可。
...
你只提取结构，不写具体操作步骤：
  节点 = 一句话描述 + complexity + section_heading + line_range + node_type
  边   = 触发条件 + 目标节点
  具体步骤由 Agent 2 负责，你不需要写

每个节点必须记录 line_range（你在 Read 时看到这段内容的行号范围）。
  这是 Agent 2 定位原文的最可靠锚点。
  即使有 section_heading，也必须同时记录 line_range。

node_type 必须从 5 种中选择（详见分类标准）：
  human_observation / api_call / remote_action / physical_action / decision
  核心判断："这个步骤需不需要人在设备旁边？"
    需要人看/听/摸 → human_observation
    需要人动手操作硬件 → physical_action
    可以远程执行命令获取信息 → api_call
    可以远程执行命令改变状态 → remote_action
    不需要执行，只需要判断 → decision
...
**`node_type` 字段**（5 种，驱动 KB entry 中的行为标签，以及 agent 运行时导航行为）：

| 值 | 含义 | Entry 标签 | Agent 行为 |
|---|---|---|---|
| `human_observation` | 用户观测物理信号（LED、声音、温度） | `[observe]` | 必须问用户 |
| `api_call` | 远程调用接口/命令获取信息（只读） | `[api]` | agent 可自动执行 |
| `remote_action` | 远程执行操作改变系统状态（重启、安装、配置） | `[remote]` | agent 可自动执行 |
| `physical_action` | 物理操作硬件（拔插、断电、更换部件） | `[physical]` | 必须告知用户去做 |
| `decision` | 基于已知信息判断走哪条分支 | `[decide]` | agent 自行判断 |

**`api_call` vs `remote_action` 区别**：`api_call` 是获取信息（查状态、跑诊断），`remote_action` 是改变状态（重启、安装、修改配置）。两者 agent 都可以自动执行，区分是为了语义清晰。

**`node_type` 分类标准**（Agent 1 System Prompt 中明确）：
...
**`line_range` 字段**：Agent 1 在通读阶段记录该节点内容在原文中的行号范围 `[start, end]`。这是 Agent 2 定位原文的第一优先级锚点，比 `section_heading` Grep 更精准，对 prose 文档（无标题）尤其重要。

**`section_heading` 字段**：Agent 2 定位原文内容的优先级链：

```
1. line_range 存在 → Read(line_range[0], line_range[1] - line_range[0]) → 直接提取
2. line_range 不存在或内容不匹配 → fallback 到 section_heading：
   Grep(section_heading) → 找到起始行号 start
   Grep("^#{同级或更高} ", offset=start+1) → 找到下一同级标题行号 end
   Read(start, end-start) → 提取完整 section 内容
3. section_heading 也为 null → fallback 到 description 关键词 Grep
4. 全部失败 → content_source: match_failed，进入 ImportReport.warnings
```
...
└── [observe] 指示灯红色闪烁 → 固件修复流程 🔧
    ├── [decide] 修复成功 → END
    └── [physical] 修复失败 → 硬件更换流程 🔧

### 分支 2：启动序列相关（4 个节点）
└── [api] 指示灯绿色，有启动尝试 → 内存诊断流程 🔧
    ├── [decide] 输出正常 → END
    ├── [decide] 输出 E01 → E01错误修复 🔧
    └── [decide] 输出 E02 → E02错误修复 🔧
...
- `[observe]`/`[api]`/`[remote]`/`[physical]`/`[decide]` = node_type 行为标签
- `section` 字段：Agent 2 定位原文内容的锚点；无此字段则用 line_range 或节点描述定位
- `line_range` 字段：原文行号范围，Agent 2 定位的第一优先级
- `END` = 终止节点，无出口
- 按根节点一级分支分区展示，每段标注节点数，便于分段 review
...
**交叉验证**（程序化 + LLM 抽检）：
- 程序化：每个 section/line_range 字段验证原文存在；无环检测
- LLM 抽检：随机抽 min(10, 节点总数) 个节点，对比：
  - 分支条件与原文语义一致性
  - node_type 与节点描述/原文内容是否一致（如"观察 LED 颜色"应为 human_observation 而非 api_call；"拔出 GPU 卡"应为 physical_action 而非 remote_action）
...
**当前目标：≤30 个 process 节点，全局视野模式**

```
≤ 30 个 process 节点：全局视野模式（当前实现）
  Phase 1 一次性读完所有 section 内容
  全部内容同时在 context → 术语天然一致
  顺序写 entry，无需 read_entry 也能保持一致

> 30 个 process 节点：分批子 agent 模式（后续迭代）
  设计保留，待有真实大规模文档后验证和实现
  基本思路：每批 10 个节点，独立 sub-agent，通过标题摘要表传递术语上下文
```
...
Agent 2 定位每个节点对应的原文内容时，按以下优先级链执行：

```
优先级 1：line_range（Agent 1 记录的行号范围）
  Read(line_range[0], line_range[1] - line_range[0]) → 直接提取
  最精准，对 prose 文档（无标题）尤其重要

优先级 2：section_heading（标题锚点）
  line_range 不存在或提取内容明显不匹配时 fallback
...
全部失败：
  write_entry 时 frontmatter 标注 content_source: match_failed
...
**第一步 [observe]：观察电源指示灯状态**

- 指示灯不亮 → **[physical]** 检查电源线连接是否松动
  - 松动 → **[physical]** 重新插紧，重新上电
...
2. **[observe]** 等待工具运行完成（约 3 分钟），观察设备面板状态指示灯
...
- **规模策略**：当前目标 ≤30 process 节点，全局视野模式（一个 agent 生成所有 entries）；分批子 agent 策略为后续迭代
...
Steps 内容质量约束（每一步必须满足）：

  1. 行为标签：每步开头必须标注 **[api]** / **[remote]** / **[physical]** / **[observe]** / **[decide]**
     标签来自 DAG 节点的 node_type，必须一致

  2. 可执行性：
     [api] 和 [remote] 步骤必须包含完整可执行的命令或 API 调用
       ✓ `nvidia-smi --query-gpu=driver_version --format=csv,noheader`
       ✗ "检查驱动版本"（不可执行）
     命令必须逐字来自原文，不得自行编造命令
     如果原文只有描述没有具体命令，写描述并标注 [command not in source]

  3. 判断条件：
     [decide] 步骤必须给出明确的条件和对应路径
       ✓ "输出 status=pass → 内存正常；输出 code=E01 → 参考 [E01 修复](...)"
       ✗ "根据结果判断下一步"（模糊）

  4. 预期输出（可选但推荐）：
     [api] 步骤如果原文提供了预期输出，必须写出来
     帮助 agent 自动判断命令是否执行成功

  5. 路由链接：
     有子分支的步骤必须使用 [标题](entry-id) 格式
     链接文案必须来自目标 entry 的真实 title（write_entry 前先 read_entry 确认）
...
内容质量校验（写入 pending 但标注 warning）：
  ⚠ 某步标注 [api] 或 [remote] 但未包含代码块或命令 → "missing executable command"
  ⚠ 某步有分支但未包含 [...](entry-id) 链接 → "missing routing link"
  ⚠ 某步缺少行为标签 → "missing behavior tag"
...
content_source 标注（非格式错误，写入 pending 但标注警告）：
  所有定位方式（line_range → section_heading → description Grep）均失败 → frontmatter 写入：
...
Holmes 不拥有 agent。Holmes 通过 MCP 暴露知识库，外部 agent（Claude Code 或任何 MCP 客户端）用自身能力执行排查步骤。**KB entry 的内容质量 = 产品价值的上限。**

### 导航流程

```
用户：我的设备上电后无法初始化
  → Agent 调用 kb_search → 找到 pitfall: 硬件初始化失败
  → Agent 调用 kb_read → 读 Resolution，看到 [observe] 标签
  → Agent 问用户："请观察电源指示灯颜色？"
  → 用户：红色闪烁
  → Agent 按路由链接找到：固件修复流程
  → Agent 调用 kb_read → 读 process entry 的 Steps
  → Step 1 标注 [remote]（执行固件重置命令）→ Agent 用 Bash 自动执行
  → Step 2 标注 [api]（检查重置结果）→ Agent 用 Bash 自动执行
  → Step 3 标注 [decide]（根据结果判断）→ Agent 自行判断
  → 判断结果指向子 entry → Agent 调用 kb_read 继续
```

### 行为标签驱动 agent 行为

每个 Steps 中的步骤都带有行为标签，agent 据此决定行为：

| 标签 | Agent 行为 | 连续自动执行 |
|---|---|---|
| `[api]` | 直接执行命令获取信息 | ✓ 可连续 |
| `[remote]` | 直接执行命令改变状态 | ✓ 可连续 |
| `[decide]` | 根据结果自行选择分支 | ✓ 可连续 |
| `[observe]` | 停下来问用户（需要人在现场） | ✗ 必须等待 |
| `[physical]` | 停下来告知用户去做（物理操作） | ✗ 必须等待 |

**连续自动执行链路**：`[api] → [remote] → [api] → [decide]` 这样的序列中，agent 一口气执行完所有步骤不打扰用户。直到遇到 `[observe]` 或 `[physical]` 才停下来。这是"给专业人员提效"的核心机制。

### CLAUDE.md 使用指引

Holmes 通过 CLAUDE.md（agent 启动时加载）提供行为标签的解读指引：
...
英文标签对任何 LLM 都自解释（`[physical]`、`[observe]` 不需要额外说明就能理解语义），CLAUDE.md 只是加强确定性。
...
| 指标 | 阈值 | 提示 |
|---|---|---|
| 总节点数 | > 30 | "链路较长，建议分阶段组织文档" |
| 最大嵌套深度 | > 5 | "嵌套较深，agent 导航可能受影响" |
| process 节点数 | > 15 | "将生成较多 entries，建议 review 关联关系" |
...
{"ts":"2026-06-23T14:36:00Z","trace":"gpu-troubleshooting","span":"lint","level":"WARN","msg":"content_source: match_failed","node_id":"N5"}
...
2026-06-23 14:30:00  [import #1]
  ...
  agent2.node[N3]  11s   entry=gpu-init-pcie-reset-001   WARN: match_failed
...
- DAG 节点 schema 扩展：
  - `node_type` 5 种：`human_observation` / `api_call` / `remote_action` / `physical_action` / `decision`
  - 新增 `line_range: [start, end]` 字段（原文行号范围，Agent 2 定位的第一优先级锚点）
- Harness 实现：
  - 工具白名单（只允许 5 个工具：Read / Grep / write_dag / read_dag / output_dag）
  - maxTurns = 300（适用所有文档类型，无需区分）
  - `output_dag` 结构校验：根节点存在、无悬空边、无环、process 节点有 line_range 或 section_heading 或描述、node_type 合法
  - `output_dag` node_type 合理性校验（warning）：物理关键词 vs non-physical 节点、查询关键词 vs remote_action 节点
  - Crash Recovery：每 20 turns 将 messages 序列化写入 `_import-state/<hash>.session.json`；`--resume` 时从快照恢复继续
- System prompt：
  - 三阶段说明（通读 → 初稿 → review）
  - node_type 分类标准和典型例子（5 种类型的判断依据）
  - line_range 必须记录要求
  - 终止 checklist 增加 line_range 和 node_type 检查
- .dag.md 树形概览增加行为标签（`[observe]`/`[api]`/`[remote]`/`[physical]`/`[decide]`），按一级分支分区标注节点数
...
- Agent 2 实现（独立 context，与 Agent 1 完全隔离）：
  ...
  - 规模策略：当前实现全局视野模式（≤30 process 节点）；分批子 agent 为后续迭代
  - 原文定位优先级链：line_range → section_heading Grep → description Grep → match_failed warning
  - Steps 内容质量约束：每步必须有行为标签（`[api]`/`[remote]`/`[physical]`/`[observe]`/`[decide]`）；`[api]`/`[remote]` 步骤必须包含可执行命令
  - write_entry 内容质量校验（warning）：缺少命令、缺少路由链接、缺少行为标签
...
- DAG 节点 schema 包含 `line_range` 和 5 种 `node_type`
- `output_dag` 结构校验通过后生成 `_import-state/<hash>.dag.md` 和 `.dag.json`
- `output_dag` node_type 合理性校验：物理关键词 vs non-physical 节点产生 warning
- 校验失败（悬空节点、循环引用、node_type 非法等）：返回 error，agent 修正后重试
- .dag.md 树形概览包含行为标签（`[observe]`/`[api]`/`[remote]`/`[physical]`/`[decide]`），按分支分区标注节点数
...
**验收条件**：
- Step 2.5 合并展示"编辑识别 + 内容验证"一屏，包含 node_type 抽检，用户一次确认
- Agent 2 生成顺序：叶节点 → 父节点 → pitfall root（拓扑逆序）
- Agent 2 原文定位按优先级链：line_range → section_heading → description Grep → match_failed
- Steps 每步标注行为标签（`[api]`/`[remote]`/`[physical]`/`[observe]`/`[decide]`），与 DAG node_type 一致
- `[api]`/`[remote]` 步骤必须包含完整可执行命令，否则标注 `[command not in source]`
- `write_entry` 结构校验失败：返回 error，agent 修正后重试；重试仍失败进 errors
- `write_entry` 内容质量校验：缺少命令/链接/标签时进 warnings（不阻断写入）
- 定位全部失败：frontmatter 标注 `content_source: match_failed` 进 warnings
