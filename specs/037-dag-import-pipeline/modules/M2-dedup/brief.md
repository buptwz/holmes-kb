...
### store.py
新增两个查询函数（搜索范围：pending 空间 `_pending/<category>/` + confirmed 空间 `<category>/`）：
...
实现方式：遍历 `kb_root` 下所有 `.md` 文件（含 `_pending/<category>/` 子目录），读取 frontmatter，比对字段值。`source_file` 比对时使用规范化后的相对路径。
...
pending 空间：_pending/<category>/*.md
```
...
- [ ] 同时搜索 `_pending/<category>/` 和 confirmed `<category>/` 两个空间
