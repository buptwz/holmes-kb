# 服务器 NVMe SSD 间歇性掉盘排查

## 背景

NPI 阶段新平台 Granite-X 服务器在长时间压力测试中出现 NVMe SSD 间歇性掉盘现象。系统运行 72-96 小时后，部分 NVMe 盘突然从操作系统中消失，无法通过 `lsblk` 或 `nvme list` 检测到。重启后恢复正常，但问题会在持续压力下复现。该问题影响了 Granite-X 平台的量产进度，需要紧急排查定位。

## 环境

- **服务器平台**: Granite-X 双路服务器 (Intel Emerald Rapids)
- **CPU**: Intel Xeon w9-3595X × 2
- **NVMe SSD**: Samsung PM9A3 3.84TB × 24 (U.2 接口)
- **NVMe 固件版本**: GDC7302Q
- **背板**: NVMe backplane Rev 2.1 (支持 24 × U.2 热插拔)
- **PCIe Switch**: Broadcom PEX89048 (48-lane PCIe Gen4 switch)
- **操作系统**: Ubuntu 22.04 LTS (kernel 5.15.0-91)
- **BIOS 版本**: 1.3.2-beta
- **BMC 固件**: OpenBMC 2.12.0
- **散热方案**: 前端 6 × 8038 风扇 + 后端 2 × 6038 风扇

该平台采用 PCIe Switch 拓扑，每个 PEX89048 管理 12 块 NVMe 盘，共 2 个 Switch 覆盖 24 块盘。Switch 通过 PCIe Gen4 x16 上联至 CPU。

## 症状

- 压力测试运行 72-96 小时后，1-3 块 NVMe SSD 突然从系统中消失
...
首先需要确认掉盘是随机的还是有规律的，收集充分的环境数据：

1. [api:read] 列出当前所有 NVMe 设备及其 PCIe 拓扑位置：
...
2. [api:read] 查看 NVMe 设备的 PCIe 链路状态：
...
6. [decide] 分析掉盘模式：
   - 如果掉盘集中在特定 slot 位置（slot 9-12 或 21-24）→ 走路径 A（散热问题）
   - 如果掉盘随机分布且伴随 PCIe AER 错误 → 走路径 B（PCIe Switch 问题）
   - 如果掉盘前 SMART critical_warning 非零 → 走路径 C（SSD 固件问题）

---

### 路径 A：散热导致的热保护掉盘

掉盘集中在 backplane 靠近电源模块侧（slot 9-12, 21-24），这些位置散热条件最差：

7. [api:read] 持续监控 NVMe 盘面温度，识别热点：
...
```
Expected: 查看所有风扇的 RPM 和当前散热策略模式（手动/自动/全速）
...
11. [verify] 72 小时后检查结果：
    - 无掉盘 → 确认为散热问题，需要优化风道设计或调整风扇曲线
    - 仍有掉盘 → 散热不是唯一原因，继续走路径 B
...
PCIe Switch PEX89048 管理的下游端口出现间歇性 link down：

13. [api:read] 查看 PCIe Switch 端口的错误计数器：

```bash
$ switchtec status /dev/switchtec0 | grep -A3 "Port"
$ switchtec port-bind-info /dev/switchtec0
```
Expected: 查看每个 Switch 下游端口的链路状态和绑定信息

14. [api:read] 导出 Switch 的详细诊断信息：

```bash
$ switchtec mfg info /dev/switchtec0
$ switchtec fw-info /dev/switchtec0
```
Expected: 查看 Switch 固件版本和制造信息

15. [api:read] 检查 Switch 上游链路（到 CPU）的状态：
...
16. [api:danger] 重置 PCIe Switch 的错误计数器，开始新一轮观察：
...
17. [api:write] 运行 24 小时压力测试，同时持续记录 Switch 事件：
...
```
Expected: 在后台记录 Switch 事件的同时运行 24 小时 NVMe 压力测试

18. [verify] 分析 switch_events.log：
...
SMART critical_warning bit 4（volatile memory backup device failed）指示 SSD 内部电容健康度问题：

19. [api:read] 批量检查所有 NVMe 盘的 SMART critical_warning 字段：
...
20. [api:read] 查看 SSD 固件版本和序列号，确认是否在已知问题批次内：
...
21. [api:read] 拉取 Samsung NVMe 的厂商特定 SMART 日志，查看内部诊断数据：
...
22. [api:danger] 对出现 critical_warning 的 SSD 进行固件在线升级：

```bash
$ nvme fw-download /dev/nvme0n1 --fw=PM9A3_GDC7502Q.bin
$ nvme fw-activate /dev/nvme0n1 --slot=1 --action=1
```
Expected: 下载新固件到 SSD 并激活到 slot 1（action=1 表示下次重启激活）

23. [verify] 固件升级后运行 72 小时 burn-in 测试，确认 critical_warning 为 0 且无掉盘

24. [decide] 如果批量 SSD 都有 critical_warning：
    - bit 4 (volatile memory backup) → 联系 Samsung FAE，确认是否为已知电容批次问题
    - bit 0 (spare space) → SSD 写入寿命问题，检查压测的写入量是否超出规格

## 根因总结

经过完整排查，该问题由两个因素叠加造成：

1. **散热设计缺陷**（主因，占比 70%）：Granite-X 的 NVMe backplane 风道设计不合理，slot 9-12 和 21-24 位于电源模块热辐射区域，长时间满载运行时 SSD 表面温度超过 70°C 触发固件的热保护掉盘。增加导风罩后 slot 间温差从 12°C 降至 4°C，问题消除。

2. **SSD 固件的热保护阈值过于激进**（次因，占比 30%）：Samsung PM9A3 固件版本 GDC7302Q 的 composite temperature 热保护阈值设为 70°C，而数据手册标称工作温度为 0-70°C。在高密度部署场景中，70°C 阈值过于激进。升级至 GDC7502Q 后阈值调整为 75°C，并增加了渐进式降速策略（70°C 开始降速，75°C 才掉盘）。

## 经验教训

1. **高密度 NVMe 平台必须做散热仿真**：在 24 盘位 U.2 平台中，靠近电源模块的 slot 位温度会比中间位高 8-15°C。NPI 阶段应在 full-load burn-in 前完成 CFD 仿真和实测温度 mapping。

2. **SSD 固件版本管理**：NPI 阶段应锁定 SSD 固件版本，并与 SSD 厂商确认已知问题列表。PM9A3 的 GDC7302Q 固件已知在高温场景有误触发热保护的问题，GDC7502Q 已修复。

3. **PCIe Switch 日志是重要诊断信息源**：掉盘不一定是 SSD 本身的问题，PCIe Switch 的 Surprise Link Down 事件可以帮助区分 SSD 故障 vs 链路层故障 vs 热保护。应在 burn-in 流程中加入 Switch 事件日志的自动采集和分析。

4. **burn-in 监控应包含温度趋势**：传统 burn-in 只关注功能通过/失败，但热相关问题往往需要 48-96 小时才会暴露。建议 burn-in 流程增加全程温度曲线记录，对斜率异常的 slot 位提前预警。

## 附录：详细排查命令参考

### NVMe 诊断命令集

以下命令在排查 NVMe 相关问题时经常使用，建议 NPI 工程师熟悉：
...
Broadcom PEX89048 PCIe Switch 的诊断需要 switchtec 命令行工具：
...
```
Expected: 显示 Switch 芯片的内部温度，正常工作范围应在 85C 以下
...
```
Expected: 每 5 分钟记录所有 NVMe 盘的温度、critical warning 和 available spare 到 CSV 文件，后台持续运行

该脚本输出的 CSV 文件可以用 Excel 或 Python matplotlib 绘制温度趋势图，快速识别热点 slot 位。建议在 burn-in 开始前启动，持续记录至 burn-in 结束。
